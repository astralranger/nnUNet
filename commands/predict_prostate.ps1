...
$output_folder = "C:\The Sketchbook\SEM VI\CV-Project\data\predictions_prostate"

If (!(Test-Path $output_folder)) {
    New-Item -ItemType Directory -Path $output_folder
}

Write-Host "Predicting Prostate segmentations using best checkpoint..."
.\venv\Scripts\nnUNetv2_predict.exe -i $input_folder -o $output_folder -d 5 -c 3d_fullres -f 0 -chk checkpoint_best.pth
